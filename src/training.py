...
def callbacks(no_early_stopping, epochs, modelname, run):
    ...
    if no_early_stopping:
        utils.rna_cdropout_print("early_stopping is disabled")
        callbacks = [mcp_save]
    else:
...
def serialize(hist, modelname, epochs, run):
    ...
    utils.rna_cdropout_print(f"Hist[{dump_file}] dumped!")


def print_times(times):
    utils.rna_cdropout_print('--------------- Timing ----------------')
    mt = times.mean()
    stdt = times.std()
    tm = f"{mt}±{stdt}"
    utils.rna_cdropout_print(f"Done in {tm} sec.")
    utils.rna_cdropout_print(f"[CSV_t] {tm}")
...
if __name__ == '__main__':
    ...
    utils.rna_cdropout_print("Stage 05: Training")
    ...
    utils.rna_cdropout_print(f"Selected Dropout: {args.dp}")
    ...
    utils.rna_cdropout_print(f"Train set loaded! Shape = {train_df.shape}")
    val_df = pd.read_csv(f"./src/data/{args.valset}", comment='#')
    utils.rna_cdropout_print(f"Validation set loaded! Shape = {val_df.shape}")
    ...
    if args.layers is None:
        ...
        args.layers = f"{l1}:{l2}"
        utils.rna_cdropout_print(f"Using layers= {args.layers}")

    device_name = tf.test.gpu_device_name()
    if args.gpu:
        device_name = args.gpu
    utils.rna_cdropout_print(f"Device Name to use: '{device_name}'")
    ...
    with tf.device(device_name):
        ...
        for i in range(args.runs):
            utils.rna_cdropout_print(f"*** Run {i} ***")
